...
function AdminSeoPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <p className="text-xs font-mono text-zinc-500 py-8">Loading SEO configurations...</p>
      ) : (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 overflow-hidden">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-zinc-800/60">
              {seoList.map((seo) => (
                <tr key={seo.id} className="hover:bg-zinc-900/40">
                  <td className="p-4 font-mono text-emerald-400 font-bold">{seo.route}</td>
                  <td className="p-4 font-semibold text-white max-w-xs truncate">
                    {seo.title || "—"}
                  </td>
                  <td className="p-4 text-zinc-400 max-w-sm truncate">{seo.description || "—"}</td>
                  <td className="p-4">
                    <span
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-xl border border-zinc-800 bg-zinc-950 p-6 text-zinc-100 shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
              <h2 className="text-lg font-bold text-white">Route SEO Configuration</h2>
