...
export function ProjectCard({ project, eager = false }: { project: Project; eager?: boolean }) {
  const role = (project as any).role || "Senior Full-Stack Engineer · UI/UX Architect";
      ...
      <TiltCard className="flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-border/80 bg-surface/80 shadow-lg backdrop-blur-md transition-all duration-500 hover:border-emerald-500/40 hover:bg-surface hover:shadow-2xl hover:shadow-emerald-950/30">
        <div>
          ...
          <div className="relative aspect-[16/10] w-full overflow-hidden bg-muted/30">
            ...
            />
            {/* Minimal gradient highlight for badge legibility without obscuring screenshot */}
            ...
            {/* Category Pill */}
            <div className="absolute top-3.5 left-3.5 rounded-full border border-emerald-500/40 bg-background/90 px-3 py-1 font-mono text-[10px] font-semibold uppercase tracking-wider text-emerald-400 backdrop-blur-md shadow-sm">
              {project.category}
            </div>
